...
from app import btnBrn, btnMenu, btnbar, btnTime
...
dataBtn = ReplyKeyboardMarkup().add(b27).add(b28).add(b29).add(b30).add(b31).add(b32).add(b33).add(b34).add(
    b35)
...
@dp.message_handler(text=btnBrn, state=None)
...
@dp.message_handler(state=FSMbron.name)
async def load_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if message.text != "❌ ОТМЕНИТЬ":
            data['name'] = message.text
            await FSMbron.next()
            await message.answer('📅 На какую дату?', reply_markup=dataBtn)
        else:
            markup = ReplyKeyboardMarkup(resize_keyboard=True)
...
@dp.message_handler(state=FSMbron.date)
async def load_date(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['date'] = message.text
        await FSMbron.next()
        await message.answer('🕗 Выберите время бронирования: ', reply_markup=timeBtn)


@dp.message_handler(state=FSMbron.time)
async def load_time(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['time'] = message.text
        await FSMbron.next()
        await message.answer('👪 На какое количество гостей?', reply_markup=pepBtn)


@dp.message_handler(state=FSMbron.people)
...
@dp.message_handler(content_types=ContentType.CONTACT, state=FSMbron.phone_number)
...
@dp.message_handler(state=FSMbron.phone_number)
async def load_phone_number(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
...
@dp.message_handler(text=btn_done)
async def cencel_message(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        markup = ReplyKeyboardMarkup(resize_keyboard=True)
        markup.row(btnMenu, btnbar, btnTime).add(btnBrn)
        await message.answer("Бронь принята\n"
...
@dp.message_handler(text=btn_tm)
async def otm(message: types.Message, state: FSMContext):
    markup = ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row(btnMenu, btnbar, btnTime).add(btnBrn)
    await message.answer("Бронь отменена", reply_markup=markup)
    await state.finish()
